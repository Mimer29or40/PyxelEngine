...
def window_pos_callback(window: int, x: int, y: int) -> None:
    ...
    print(f"{counter:08X} to {slot.number} at {glfw.get_time():0.3f}: Window position: {x}, {y}")
...
def window_size_callback(window: int, width: int, height: int) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08X} to {slot.number} at {glfw.get_time():0.3f}: Window size: {width}, {height}")
...
def framebuffer_size_callback(window: int, width: int, height: int) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08X} to {slot.number} at {glfw.get_time():0.3f}: Framebuffer size: {width}, {height}")
...
def window_content_scale_callback(window: int, xscale: float, yscale: float) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08X} to {slot.number} at {glfw.get_time():0.3f}: Window content scale: {xscale:0.3f}, {yscale:0.3f}")
...
def window_close_callback(window: int) -> None:
    ...
    if not slot.closable:
        print(f"(( closing is disabled, press {glfw.get_key_name(glfw.KEY_C, 0)} to re-enable )")
...
def window_focus_callback(window: int, focused: bool) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Window {'focused' if focused else 'unfocused'}")
...
def window_iconify_callback(window: int, iconified: bool) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Window was {'iconified' if iconified else 'uniconified'}")
...
def window_maximize_callback(window: int, maximized: bool) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Window was {'maximized' if maximized else 'unmaximized'}")
...
def mouse_button_callback(window: int, button: int, action: int, mods: int) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Mouse button {button} ({get_button_name(button)}) (with{get_mods_name(mods)}) was {get_action_name(action)}")
...
def cursor_position_callback(window: int, x: float, y: float) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Cursor position: {x} {y}")
...
def cursor_enter_callback(window: int, entered: bool) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Cursor {'entered' if entered else 'left'} window")
...
def scroll_callback(window: int, x: float, y: float) -> None:
    ...
    slot = glfw.get_window_user_pointer(window)
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Scroll: {x:0.3f} {y:0.3f}")
...
def key_callback(window: int, key: int, scancode: int, action: int, mods: int) -> None:
    ...
    if name:
        print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Key 0x{key:04x} Scancode 0x{scancode:04x} ({get_key_name(key)}) ({name}) (with{get_mods_name(mods)}) was {get_action_name(action)}")
    else:
        print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Key 0x{key:04x} Scancode 0x{scancode:04x} ({get_key_name(key)}) (with{get_mods_name(mods)}) was {get_action_name(action)}")
...
def char_callback(window: int, codepoint: int) -> None:
    ...
    print(f"{counter:08x} to {slot.number} at {glfw.get_time():0.3f}: Character 0x{codepoint:08x} ({string}) input")
...
def monitor_callback(monitor: int, event: int) -> None:
    ...
    if event == glfw.CONNECTED:
        ...
        print(f"{counter:08x} at {glfw.get_time():0.3f}: Monitor {glfw.get_monitor_name(monitor)} ({mode.width}x{mode.height} at {x}x{y}, {widthMM}x{heightMM} mm) was connected")
        
        counter += 1
    elif event == glfw.DISCONNECTED:
        print(f"{counter:08x} at {glfw.get_time():0.3f}: Monitor {glfw.get_monitor_name(monitor)} was disconnected")
...
def joystick_callback(jid: int, event: int) -> None:
    ...
    if event == glfw.CONNECTED:
        ...
        print(f"{counter:08x} at {glfw.get_time():0.3f}: Joystick {jid} ({glfw.get_joystick_name(jid)}) was connected with {axisCount} axes, {buttonCount} buttons, and {hatCount} hats")
        
        if glfw.joystick_is_gamepad(jid):
            print(f"  Joystick {jid} ({glfw.get_joystick_guid(jid)}) has a gamepad mapping ({glfw.get_gamepad_name(jid)})")
        else:
            print(f"  Joystick {jid} ({glfw.get_joystick_guid(jid)}) has no gamepad mapping")
    else:
        print(f"{counter:08x} at {glfw.get_time():0.3f}: Joystick {jid} was disconnected")
...
def main():
    ...
    for i in range(count):
        ...
        if monitor is not None:
            print(f"Creating full screen window {slot.number} ({width}x{height} on {glfw.get_monitor_name(monitor)})")
        else:
        ...
        glfw.set_window_content_scale_callback(slot.window, window_content_scale_callback)
