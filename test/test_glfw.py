...
from PyxelEngine.io.monitor import GammaRamp
from PyxelEngine.io.monitor import Monitor
from PyxelEngine.io.monitor import VideoMode
...
class TestMonitor(unittest.TestCase):
    ...
    def test_getters(self) -> None:
        ...
        self.assertIsInstance(
            primary, Monitor, "Primary Monitor must be of type Monitor"
        )
        ...
        self.assertIs(
            Monitor.get_primary(),
            Monitor.get_index(0),
            "Monitor at 0 index must be primary",
        )
    ...
    def test_video_mode(self) -> None:
        ...
        self.assertIsInstance(
            primary, VideoMode, "Primary VideoMode must be of type VideoMode"
        )

        video_mode: VideoMode = monitor.video_mode
        self.assertIsNotNone(primary, "Current VideoMode cannot be None")
        self.assertIsInstance(
            primary, VideoMode, "Current VideoMode must be of type VideoMode"
        )
        self.assertEqual(
            primary, video_mode, "Primary VideoMode must be the initial VideoMode"
        )
        ...
        self.assertIsInstance(
            video_mode.width, int, "VideoMode.width must be of type int"
        )
        ...
        self.assertIsInstance(
            video_mode.height, int, "VideoMode.height must be of type int"
        )
        ...
        self.assertIsInstance(
            video_mode.green, int, "VideoMode.green must be of type int"
        )
        ...
        self.assertIsInstance(
            video_mode.blue, int, "VideoMode.blue must be of type int"
        )
        self.assertGreater(video_mode.blue, 0, "VideoMode.blue must be > 0")

        self.assertIsNotNone(
            video_mode.refresh_rate, "VideoMode.refresh_rate cannot be None"
        )
        self.assertIsInstance(
            video_mode.refresh_rate, int, "VideoMode.refresh_rate must be of type int"
        )
        self.assertGreater(
            video_mode.refresh_rate, 0, "VideoMode.refresh_rate must be > 0"
        )
    ...
    def test_gamma_ramp(self) -> None:
        ...
        self.assertIsInstance(
            gamma_ramp.size, int, "GammaRamp.size must be of type int"
        )
        ...
        self.assertIsInstance(
            gamma_ramp.red, np.ndarray, "GammaRamp.red must be of type np.ndarray"
        )
        self.assertGreaterEqual(
            len(gamma_ramp.red),
            gamma_ramp.size,
            "len(GammaRamp.red) must be >= GammaRamp.size",
        )

        self.assertIsNotNone(gamma_ramp.green, "GammaRamp.green cannot be None")
        self.assertIsInstance(
            gamma_ramp.green, np.ndarray, "GammaRamp.green must be of type np.ndarray"
        )
        self.assertGreaterEqual(
            len(gamma_ramp.green),
            gamma_ramp.size,
            "len(GammaRamp.green) must be >= GammaRamp.size",
        )

        self.assertIsNotNone(gamma_ramp.blue, "GammaRamp.blue cannot be None")
        self.assertIsInstance(
            gamma_ramp.blue, np.ndarray, "GammaRamp.blue must be of type np.ndarray"
        )
        self.assertGreaterEqual(
            len(gamma_ramp.blue),
            gamma_ramp.size,
            "len(GammaRamp.blue) must be >= GammaRamp.size",
        )
